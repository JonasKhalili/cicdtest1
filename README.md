...
```
dbx execute --cluster-name=<name of interactive cluster> --job=cicdtest1-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=cicdtest1-sample-integration-test --files-only
dbx launch --job=cicdtest1-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `cicdtest1/__init__.py` file and tag the current code version:
```
